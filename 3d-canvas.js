...
//an array of pixels with 3D coordinates
var pixels = [];
...
function render()
{
	...
	while(i--)
	{
		...
		//fov = field of view = denotes how far the pixels are from us. The scale will control 
		//how the spacing between the pixels will decrease with increasing distance from us. 
		...
		{
			//Imagedata.width gives the width of the captured region(canvas) which is multiplied with the Y
			//cooridinate and then added to the X coordinate. The whole thing is
			//multiplied gives the first color data(red) for the pixel. 
			var c = (Math.round(y2d) * imagedata.width + Math.round(x2d)) * 4;
			imagedata.data[c] = 0; //red
			imagedata.data[c + 1] = 255; //green
			imagedata.data[c + 2] = 200; //blue
